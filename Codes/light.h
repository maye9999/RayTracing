...
public:
	...
	virtual bool computeShadow(const LocalGeometry& localGeo, Ray& light_ray, Color& light_color)
	{
		...
		bool found = false;
		if(scene->kd_tree != nullptr)	// Use Kd_Tree
	...
	virtual bool computeShadow(const LocalGeometry& localGeo, Ray& light_ray, Color& light_color)
	{
		...
		for(int i = 0; i < samples; ++i)
		{
			for(int j = 0; j < samples; ++j)
			{
				...
				double r = (double)rand() / RAND_MAX * div;	// Randomly choose a point from the grid
				p = p + r * u;
		...
		if(through_times == 0)		// nothing hits
		{
			return true;
		}
		...
		light_color = double(through_times) / samples / samples * color;	// Use the ratio to be light color
		return false;
	}
	// Whether a light intersect with a plane
